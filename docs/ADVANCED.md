...
|**Codex**|`codex`| `CODEX_BIN`, `CODEX_PROFILE`, `CODEX_MODEL`, `CODEX_REASONING_EFFORT`, `CODEX_TIMEOUT` | `codex`, ``, ``, `none`, `120` | Uses the local Codex CLI login state. No API key required. With profile/model blank, the isolated fast path delegates model choice to the CLI/account recommendation (currently normally `gpt-5.6-sol`, not pinned); blank effort is explicitly forced to `none`. `--thread` controls the maximum number of independent Codex requests that may run concurrently; the WebUI starts at `1` but keeps the field editable. |
