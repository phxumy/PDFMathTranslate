...
|**Codex**|`codex`| `CODEX_BIN`, `CODEX_PROFILE`, `CODEX_MODEL`, `CODEX_REASONING_EFFORT`, `CODEX_TIMEOUT` | `codex`, ``, ``, `none`, `120` | Uses the local Codex CLI login state. No API key required. By default it uses the current CLI model selection. `--thread` controls the maximum number of independent Codex requests that may run concurrently; the WebUI starts at `1` but keeps the field editable. |
