...
|**Codex**|`codex`| `CODEX_BIN`, `CODEX_PROFILE`, `CODEX_MODEL`, `CODEX_TIMEOUT` | `codex`, ``, ``, `120` | Uses the local Codex CLI login state. No API key required. By default it uses the current CLI model selection, and effective concurrency is fixed to `1`. |
